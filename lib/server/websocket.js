...
const logIONamespaceInfo = io => {
  // 获取所有命名空间
  const allNamespaces = io._nsps;
  const arr = [];
  allNamespaces.forEach((namespace, nspName) => {
    const sockets = [];
    namespace.sockets.forEach((socket, sktName) => {
      const obj = {
        socketName: sktName,
        _eventsCount: socket._eventsCount,
        _events: Object.keys(socket._events)
      };
      sockets.push(obj);
    });
    arr.push({
      namespace: nspName,
      _events: Object.keys(namespace._events),
      'sockets.size': namespace.sockets.size,
      sockets
    });
  });
  return arr;
...
function init (env, ctx, server) {
  ...
  function listeners () {
    io.sockets.on('connection', function onConnection (socket) {
      socket.on('io info message', (data, callback) => {
        if(callback) {
          callback(logIONamespaceInfo(io));
          // callback({ result: 'success123' });
        }
      })
